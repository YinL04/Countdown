...
class WeatherAgent:
    ...
    def run(self, city: str) -> AgentAnswer:
        ...
        if self.client:
            llm_result = self._generate_with_llm(report)
            if llm_result:
                return llm_result

        return self._generate_rule_based(report)
...
{{
  ...
  "attractions": ["景点1：说明", "景点2：说明", "景点3：说明"]
}}
...
""".strip()
        ...
        try:
            ...
            return AgentAnswer(
                ...
                attractions=rule_result.attractions,
            )

    def _generate_rule_based(self, report: WeatherReport) -> AgentAnswer:
